...
func (bh *BalanceHandler) Index(w http.ResponseWriter, r *http.Request) {
	var (
	...
	data, count, err := bh.br.GetBalanceList(ctx, filter, userId)
	if err != nil {
		bh.log.Info("failed to get balance with filter", zap.Error(err))
		(&response.Response{
			HttpStatus: http.StatusInternalServerError,
			Message:    err.Error(),
		}).GenerateResponse(w)
